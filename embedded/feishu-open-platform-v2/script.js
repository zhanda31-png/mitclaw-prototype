...
const PUBLISH_TRANSITION_DELAY = 5000;
    ...
    versionHistory: [
      {
        version: "1.2.0",
        owner: "Wei He 何伟",
        publishedAt: "2026-03-12T09:42:00+08:00",
      },
    ],
...
const EXTERNAL_ROUTE_APP_SEEDS = [
  ...
  { id: "AGT002", name: "东南亚销售虾", employeeNo: "100234", miId: "13900139002", department: "东南亚销售部", owner: "杨帆", trainer: "Wei He 何伟", lifecycleStatus: "上岗" },
...
function buildExternalRouteApp(appId) {
  ...
  const now = "2026-05-07T10:00:00+08:00";
  ...
  return normalizeApp({
    ...
    version: status === "待上线" ? "1.0.0" : "1.0.1",
    versionHistory:
      status === "待上线"
        ? []
        : [
    ...
    activity: [
      {
        title: "打开岗位虾配置",
        description: `当前查看 ${seed.name} 的 ${seed.lifecycleStatus} 配置。`,
        time: now,
      },
    ],
  });
  ...
  if (nextApp) {
    ...
    if (existingIndex >= 0) {
      ...
      state.apps[existingIndex] = normalizeApp({
        ...
        lifecycleStatus: nextApp.lifecycleStatus,
      });
      saveApps();
      return true;
    }
...
const LIFECYCLE_STATUS_META = {
  上岗: {
    className: "is-lifecycle-active",
    displayText: "上岗",
  },
  试运行: {
    className: "is-lifecycle-trial",
    displayText: "试运行",
  },
  申请中: {
    className: "is-lifecycle-applying",
    displayText: "申请中",
  },
  冻结: {
    className: "is-lifecycle-frozen",
    displayText: "冻结",
  },
  下岗: {
    className: "is-lifecycle-offboarded",
    displayText: "下岗",
  },
};
...
const DETAIL_NAV_GROUPS = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: "cron-management", label: "Cron管理", icon: "cron" },
...
const VERSION_CHANGE_GROUPS = [
  {
    sectionLabel: "岗位说明书",
    items: [
      { label: "岗位名称", getValue: (snapshot) => normalizeTrackedText(snapshot.name) },
      { label: "岗位职责", getValue: (snapshot) => normalizeTrackedText(snapshot.description) },
      { label: "所属部门", getValue: (snapshot) => normalizeTrackedText(snapshot.department) },
      { label: "核心场景", getValue: (snapshot) => normalizeTrackedText(snapshot.coreScenario) },
      { label: "容器需求", getValue: (snapshot) => normalizeTrackedText(snapshot.containerNeeds) },
    ],
  },
  {
    sectionLabel: "Skill管理",
    items: [
      {
        label: "机器人",
        getValue: (snapshot) => (snapshot.capabilities?.bot ? "已启用" : "未启用"),
      },
      {
        label: "网页应用",
        getValue: (snapshot) => (snapshot.capabilities?.web ? "已启用" : "未启用"),
      },
      {
        label: "工作台小组件",
        getValue: (snapshot) => (snapshot.capabilities?.widget ? "已启用" : "未启用"),
      },
    ],
  },
];
...
const state = {
  ...
  spaceManageId: "",
  versionComposerOpen: false,
  versionDetailReadonly: false,
  versionHistoryExpanded: true,
  versionReturnToSection: "baseinfo",
  versionDraft: null,
  versionNotifyPickerOpen: false,
};
...
function normalizeMemberArray(value) {
  return uniqueMemberNames(
    Array.isArray(value) ? value.map((item) => String(item || "").trim()) : [],
  );
}

function capturePublishedSnapshot(app) {
  return {
    name: normalizeTrackedText(app.name),
    description: normalizeTrackedText(app.description),
    department: normalizeTrackedText(app.department),
    coreScenario: normalizeTrackedText(app.coreScenario),
    containerNeeds: normalizeTrackedText(app.containerNeeds),
    capabilities: {
...
function normalizePublishedSnapshot(snapshot, fallback) {
  if (!snapshot || typeof snapshot !== "object") {
    return fallback;
  }

  return {
    name: normalizeTrackedText(snapshot.name ?? fallback.name),
    description: normalizeTrackedText(snapshot.description ?? fallback.description),
    department: normalizeTrackedText(snapshot.department ?? fallback.department),
    coreScenario: normalizeTrackedText(snapshot.coreScenario ?? fallback.coreScenario),
    containerNeeds: normalizeTrackedText(snapshot.containerNeeds ?? fallback.containerNeeds),
    capabilities: {
      bot:
        typeof snapshot.capabilities?.bot === "boolean"
          ? snapshot.capabilities.bot
          : fallback.capabilities.bot,
      web:
        typeof snapshot.capabilities?.web === "boolean"
          ? snapshot.capabilities.web
          : fallback.capabilities.web,
      widget:
        typeof snapshot.capabilities?.widget === "boolean"
          ? snapshot.capabilities.widget
          : fallback.capabilities.widget,
    },
  };
}
...
function normalizeVersionHistory(value, currentVersion, fallbackOwner) {
  const normalizedEntries = Array.isArray(value)
    ? value
        .map((item) => {
          ...
          return {
            version,
            owner: String(item?.owner || fallbackOwner || "Wei He 何伟").trim() || fallbackOwner,
...
function normalizeApp(app) {
  const currentSnapshot = capturePublishedSnapshot(app);
  const [colorA, colorB] =
  ...
  const pendingPublishChanges = normalizeBooleanFlag(app.pendingPublishChanges);
  const publishedSnapshot =
    !pendingPublishChanges && app.status === "已启用"
      ? currentSnapshot
      : normalizePublishedSnapshot(app.publishedSnapshot, currentSnapshot);
  ...
  const normalizedApp = {
    ...
    versionNoticeMembers: normalizeMemberArray(app.versionNoticeMembers),
    status: STATUS_META[app.status] ? app.status : "开发中",
    lifecycleStatus: getAppLifecycleStatus(app),
    publishStage: app.publishStage === "publishing" ? "publishing" : "",
    publishCompleteAt:
      app.publishStage === "publishing" ? normalizeDateValue(app.publishCompleteAt) : "",
    version: app.version || "0.1.0",
    versionHistory: normalizeVersionHistory(app.versionHistory, app.version || "0.1.0", app.owner),
  ...
  normalizedApp.versionDraft = normalizeVersionDraftConfig(app.versionDraft, normalizedApp);
...
function getStatusDisplayText(status) {
  return getStatusMeta(status).displayText || status;
}

function normalizeLifecycleStatusValue(value) {
  const normalized = String(value || "").trim();
  return LIFECYCLE_STATUS_META[normalized] ? normalized : "";
}

function getAppLifecycleStatus(app) {
  const lifecycleStatus = normalizeLifecycleStatusValue(app?.lifecycleStatus);
  if (lifecycleStatus) {
  ...
  if (app?.pendingPublishChanges && status === "待上线") {
    return "上岗";
  }

  if (status === "已启用" || status === "上岗") {
    return "上岗";
  }

  if (status === "待上线" || status === "试运行") {
    return "试运行";
  }
...
function getLifecycleStatusMeta(lifecycleStatus) {
  return (
    LIFECYCLE_STATUS_META[normalizeLifecycleStatusValue(lifecycleStatus)] ||
    LIFECYCLE_STATUS_META["申请中"]
  );
}

function getAppLifecycleMeta(app) {
  const lifecycleStatus = getAppLifecycleStatus(app);
  return {
    ...getLifecycleStatusMeta(lifecycleStatus),
    displayText: lifecycleStatus,
  };
}

function isAppPublishing(app) {
  return app?.publishStage === "publishing";
}

function clearPublishCompletionTimer(appId) {
  const timerId = publishCompletionTimers.get(appId);
  if (timerId) {
    window.clearTimeout(timerId);
    publishCompletionTimers.delete(appId);
  }
}

function completePublishTransition(appId) {
  clearPublishCompletionTimer(appId);
  const currentApp = state.apps.find((app) => app.id === appId);

  if (!currentApp || !isAppPublishing(currentApp)) {
    return;
  }

  const updated = updateAppById(appId, (current) => {
    if (!isAppPublishing(current)) {
      return current;
    }

    current.publishStage = "";
    current.publishCompleteAt = "";
    current.updatedAt = new Date().toISOString();
    appendActivity(current, "版本发布生效", `版本 ${current.version} 已成功发版。`);
    return current;
  });

  if (updated) {
    toast(`版本 ${updated.version} 已成功发版。`, "版本管理");
  }
}

function schedulePublishCompletion(appId) {
  clearPublishCompletionTimer(appId);
  const app = state.apps.find((item) => item.id === appId);

  if (!app || !isAppPublishing(app)) {
    return;
  }

  const completeAt = new Date(app.publishCompleteAt || "").getTime();
  if (Number.isNaN(completeAt)) {
    completePublishTransition(appId);
    return;
  }

  const remaining = Math.max(0, completeAt - Date.now());
  const timerId = window.setTimeout(() => completePublishTransition(appId), remaining);
  publishCompletionTimers.set(appId, timerId);
}
...
function getPondLifecycleStatus(appOrStatus) {
  const lifecycleStatus =
    typeof appOrStatus === "object" && appOrStatus !== null
      ? getAppLifecycleStatus(appOrStatus)
      : getAppLifecycleStatus({ status: appOrStatus });

  if (lifecycleStatus === "申请中") {
    return "draft";
  }

  if (lifecycleStatus === "试运行") {
    return "trial";
  }

  if (lifecycleStatus === "上岗") {
    return "active";
  }

  if (lifecycleStatus === "冻结") {
    return "frozen";
  }

  if (lifecycleStatus === "下岗") {
    return "offboarded";
  }
...
function renderPondStatusOverview() {
  ...
  getVisibleApps().forEach((app) => {
    const lifecycleStatus = getPondLifecycleStatus(app);
...
function getRestrictedNames(app) {
  return Array.isArray(app.collaborators)
    ? uniqueMemberNames(
        app.collaborators
          .filter((item) => item.role === "限制用户/部门")
          .map((item) => item.name),
      )
    : [];
}
...
function getNextVersion(version) {
  const [major, minor, patch] = String(version || "0.1.0")
    .split(".")
    .map((part) => Number(part) || 0);
  return `${major}.${minor}.${patch + 1}`;
}

function normalizeVersionDraftConfig(draft, app) {
  const previousVersion = app.version || "0.1.0";
  const defaultVersion = getNextVersion(previousVersion);
  const notificationMembersSource =
    Array.isArray(draft?.notificationMembers) && draft.notificationMembers.length
      ? draft.notificationMembers
      : app.versionNoticeMembers?.length
        ? app.versionNoticeMembers
        : getManagerNames(app);

  return {
    appId: app.id,
    version: String(draft?.version || defaultVersion).trim() || defaultVersion,
    previousVersion,
    releaseNotes: String(draft?.releaseNotes || "更新应用版本"),
    notificationMembers: normalizeMemberArray(notificationMembersSource),
  };
}

function markPendingPublishChange(app) {
  if (!app.publishedSnapshot) {
    app.publishedSnapshot = capturePublishedSnapshot(app);
  }

  app.pendingPublishChanges = true;
...
function buildVersionDraft(app) {
  return normalizeVersionDraftConfig(app.versionDraft, app);
}

function ensureVersionDraft(app) {
  if (!state.versionDraft || state.versionDraft.appId !== app.id) {
    state.versionDraft = buildVersionDraft(app);
  }

  return state.versionDraft;
}

function getVersionChangeGroups(app) {
  const beforeSnapshot = app.publishedSnapshot || capturePublishedSnapshot(app);
  const afterSnapshot = capturePublishedSnapshot(app);

  return VERSION_CHANGE_GROUPS.map((group) => {
    const changes = group.items
      .map((item) => {
        const beforeValue = item.getValue(beforeSnapshot);
        const afterValue = item.getValue(afterSnapshot);
        ...
        return {
          label: item.label,
          beforeValue: beforeValue || "暂无",
          afterValue: afterValue || "暂无",
        };
      })
      .filter(Boolean);
...
function getVersionChangeGroupBySection(app, sectionId) {
  const sectionMeta = getDetailSectionMeta(sectionId);
  if (!sectionMeta?.label) {
    return null;
  }

  return getVersionChangeGroups(app).find((group) => group.sectionLabel === sectionMeta.label) || null;
}

function getVersionNotifyCandidates(app) {
  const sourceGroups = [
    { roleLabel: "管理者", names: getManagerNames(app) },
    { roleLabel: "训虾员", names: getTrainerNames(app) },
    { roleLabel: "限制用户/部门", names: getRestrictedNames(app) },
  ];
  ...
  sourceGroups.forEach((group) => {
    group.names.forEach((name) => {
      ...
      if (!candidateMap.has(memberName)) {
        candidateMap.set(memberName, {
          name: memberName,
          roleLabels: [],
        });
      }

      candidateMap.get(memberName).roleLabels.push(group.roleLabel);
    });
  });

  return Array.from(candidateMap.values()).map((candidate) => ({
    name: candidate.name,
    roleLabels: uniqueMemberNames(candidate.roleLabels),
  }));
}

function hasPendingVersionPublish(app) {
  return Boolean(app?.pendingPublishChanges) || app?.status === "待上线";
}

function renderDetailPublishToast(app) {
  if (!detailPublishToast) {
    return;
  }

  if (isAppPublishing(app)) {
    detailPublishToast.className = "publish-toast publish-toast--success";
    detailPublishToast.innerHTML = `
      <span class="publish-toast-icon">✓</span>
      <span class="publish-toast-message">发版中，预计5分钟后生效</span>
    `;
    detailPublishToast.classList.remove("is-hidden");
    return;
  }

  if (hasPendingVersionPublish(app)) {
    const currentSectionChangeGroup = getVersionChangeGroupBySection(app, state.detailSection);
    const currentChangeCount = currentSectionChangeGroup?.changes?.length || 0;

    if (!currentChangeCount) {
      detailPublishToast.className = "publish-toast publish-toast--warning is-hidden";
      detailPublishToast.innerHTML = "";
      return;
    }

    const changeListMarkup =
      currentSectionChangeGroup?.changes?.length
        ? `
          <ul class="publish-toast-change-list">
            ${currentSectionChangeGroup.changes
              .map(
                (change) => `
                  <li class="publish-toast-change-item">
                    <span class="publish-toast-change-item__label">${escapeHtml(change.label)}：</span>
                    <span>${escapeHtml(change.beforeValue)} → ${escapeHtml(change.afterValue)}</span>
                  </li>
                `,
              )
              .join("")}
    ...
    detailPublishToast.className = "publish-toast publish-toast--warning";
    detailPublishToast.innerHTML = `
      <div class="publish-toast-main">
        <span class="publish-toast-icon" aria-hidden="true">
          <svg viewBox="0 0 16 16" fill="none">
            <path d="M8 2.2 14 13H2L8 2.2Z" stroke="currentColor" stroke-width="1.35" stroke-linejoin="round"/>
            <path d="M8 6v3.2" stroke="currentColor" stroke-width="1.35" stroke-linecap="round"/>
            <circle cx="8" cy="11.2" r=".8" fill="currentColor"/>
          </svg>
        </span>
        <div class="publish-toast-copy">
          <div class="publish-toast-message">当前有 ${currentChangeCount || 1} 项变更待发布，变更需在 <span class="publish-toast-message__strong">【版本管理与发布】</span> 中执行发布后方可生效。</div>
          ${changeListMarkup}
        </div>
      </div>
      <div class="publish-toast-actions">
        <button class="publish-toast-link publish-toast-link--primary" type="button" data-action="open-version-from-reminder">前往发版</button>
        <button class="publish-toast-link publish-toast-link--secondary" type="button" data-action="revert-pending-changes">撤回</button>
      </div>
    `;
    detailPublishToast.classList.remove("is-hidden");
    return;
  ...
  detailPublishToast.innerHTML = `
    <span class="publish-toast-icon">✓</span>
    <span class="publish-toast-message">当前修改均已成功发版</span>
  `;
  detailPublishToast.classList.remove("is-hidden");
}
...
function shouldUseHistoryHashNavigation() {
...
function navigateToList(replace = false) {
  ...
  if (shouldUseHistoryHashNavigation()) {
    if (currentHash === hash) {
      applyRouteFromHash();
      return;
    }
...
function handleOfflineNavigateMessage(event) {
  ...
  const section = String(message.payload?.section || "baseinfo").trim() || "baseinfo";

  if (!appId) {
    return;
  }

  navigateToDetail(appId, section, true);
}

function notifyOfflineRouteChange(appId, section, extraPayload = {}) {
  ...
  try {
    ...
    // 单文件 HTML 下无法通知父层时，保留当前 iframe 状态即可
  }
}

function openHostVersionDetail(appId) {
  if (window.parent === window || !appId) {
    return false;
  }
  ...
  try {
    ...
    if (parentUrl.pathname.includes("/embedded/")) {
      return false;
    }
    ...
    parentUrl.search = new URLSearchParams({
      ...
      section: "version",
      versionView: "detail",
      returnTo: state.versionReturnToSection || "baseinfo",
    }).toString();
...
function openVersionComposer(appId) {
  const app = state.apps.find((item) => item.id === appId);
  if (!app) {
    return;
  }

  state.versionReturnToSection = state.detailSection || "baseinfo";
  state.versionComposerOpen = true;
  state.versionDetailReadonly = false;
  state.versionNotifyPickerOpen = false;
  state.versionDraft = buildVersionDraft(app);

  if (openHostVersionDetail(appId)) {
    return;
  }

  notifyOfflineRouteChange(appId, "version", {
    versionView: "detail",
    returnTo: state.versionReturnToSection || "baseinfo",
  });

  navigateToDetail(appId, "version");
}

function closeVersionComposer() {
  state.versionComposerOpen = false;
  state.versionDetailReadonly = false;
  state.versionNotifyPickerOpen = false;
  state.versionDraft = null;
}

function restoreCurrentAppToPublishedVersion(app) {
  const publishedSnapshot = normalizePublishedSnapshot(
    app.publishedSnapshot,
    capturePublishedSnapshot(app),
  );

  app.name = publishedSnapshot.name;
  app.description = publishedSnapshot.description;
  app.department = publishedSnapshot.department;
  app.coreScenario = publishedSnapshot.coreScenario;
  app.containerNeeds = publishedSnapshot.containerNeeds;
  app.capabilities = {
    ...app.capabilities,
    ...publishedSnapshot.capabilities,
  };
  app.pendingPublishChanges = false;
  app.publishStage = "";
  app.publishCompleteAt = "";
  app.versionDraft = null;
  if (app.status === "待上线") {
    app.status = "已启用";
  }
...
function applyRouteFromHash() {
  ...
  if (route.view === "list") {
    ...
    state.versionComposerOpen = false;
    state.versionDetailReadonly = false;
    state.versionHistoryExpanded = true;
    render();
    return;
  }
  ...
  notifyOfflineRouteChange(
    route.appId,
    route.section,
    route.section === "version" && state.versionComposerOpen
      ? {
          versionView: "detail",
          returnTo: state.versionReturnToSection || "baseinfo",
        }
...
function renderSidebar(app) {
  ...
  const lifecycleMeta = getAppLifecycleMeta(app);
    ...
    <div class="detail-sidebar-shell">
      <section class="detail-sidebar-card detail-sidebar-intro">
        ...
        <div class="detail-sidebar-title-row">
          ...
          <span class="status-pill ${lifecycleMeta.className}">${escapeHtml(lifecycleMeta.displayText)}</span>
        </div>
...
function getVersionDisplayStatus(status) {
  const app = typeof status === "object" && status !== null ? status : { status };

  if (isAppPublishing(app)) {
    return { label: "发版中", className: "is-pending" };
  }

  if (app.status === "已启用") {
    return { label: "已发版", className: "is-published" };
  }
...
function renderVersionOwner(owner) {
  return `
    <div class="version-owner-cell">
      <span class="mini-avatar version-avatar">${escapeHtml(getCollaboratorInitial(owner))}</span>
      <span>${escapeHtml(owner)}</span>
    </div>
  `;
}

function renderVersionChangeContent(changeGroups) {
  if (!changeGroups.length) {
    return "";
  }

  return `
    <section class="version-change-block">
      <h2>变更内容</h2>
      <div class="version-change-groups">
        ${changeGroups
          .map(
            (group) => `
              <section class="version-change-group">
                <div class="version-change-group__head">
                  <span class="version-change-group__head-label version-change-group__head-label--section">${escapeHtml(group.sectionLabel)}</span>
                  <span class="version-change-group__head-label">变更前</span>
                  <span class="version-change-group__head-spacer" aria-hidden="true"></span>
                  <span class="version-change-group__head-label">变更后</span>
                </div>
                <div class="version-change-items">
                  ${group.changes
                    .map(
                      (change) => `
                        <div class="version-change-item">
                          <div class="version-change-item__label">${escapeHtml(change.label)}</div>
                          <div class="version-change-item__values">
                            <div class="version-change-value version-change-value--before">
                              <span class="version-change-value__text">${escapeHtml(change.beforeValue)}</span>
                            </div>
                            <span class="version-change-arrow">→</span>
                            <div class="version-change-value version-change-value--after">
                              <span class="version-change-value__text">${escapeHtml(change.afterValue)}</span>
                            </div>
                          </div>
                        </div>
                      `
...
function renderVersionNotifyBlock(app) {
  const managerNames = getManagerNames(app);

  return `
    <section class="version-notify-block">
      <div class="version-notify-block__head">
        <h2>将飞书通知</h2>
      </div>

      <div class="version-notify-summary">
        <span class="version-notify-summary__scope">${managerNames.length ? `已选 ${managerNames.length} 人` : "未设置"}</span>
      </div>

      <div class="version-notify-members">
        <span class="version-notify-members__label">管理员：</span>
        ${
          managerNames.length
            ? `
              <div class="version-notify-chip-list">
                ${managerNames
                  .map(
                    (name) => `
                      <div class="version-notify-chip version-notify-chip--static">
                        <span class="version-notify-chip__avatar">${escapeHtml(getCollaboratorInitial(name))}</span>
                        <span class="version-notify-chip__name">${escapeHtml(name)}</span>
                      </div>
                    `
                  )
                  .join("")}
            ...
            `
            : '<span class="version-notify-empty">暂无管理员</span>'
        }
      </div>
    </section>
  `;
...
function renderVersionComposer(app) {
  const draft = ensureVersionDraft(app);
  const changeGroups = getVersionChangeGroups(app);
  const changeContentMarkup = renderVersionChangeContent(changeGroups);
  const notifyBlockMarkup = renderVersionNotifyBlock(app);

  return `
    <article class="info-card page-card version-detail-page-card">
      <button class="version-detail-back" type="button" data-action="back-version-list">
        <span class="version-detail-back__icon">‹</span>
        <span>返回</span>
      </button>

      <div class="version-detail-page-card__head">
        <h1>版本详情</h1>
      </div>

      <div class="version-detail-notice">
        <span class="version-detail-notice__icon">✓</span>
        <span>本次发布免审核，提交发布后即可上线使用</span>
      </div>

      <div class="version-detail-form">
        <label class="version-detail-field version-detail-field--full">
          <span class="field-label">应用版本号 <span class="required-mark">*</span></span>
          <input
            type="text"
            value="${escapeHtml(draft.version)}"
            data-version-field="version"
          />
          <span class="field-tip">对用户展示的正式版本号，上一个版本号是 ${escapeHtml(draft.previousVersion)}</span>
        </label>

        <label class="version-detail-field version-detail-field--full version-detail-field--notes">
          <span class="field-label">更新说明 <span class="required-mark">*</span></span>
          <textarea
            rows="6"
            maxlength="500"
            data-version-field="releaseNotes"
          >${escapeHtml(draft.releaseNotes)}</textarea>
          <span class="version-detail-counter" data-version-counter>${draft.releaseNotes.length}/500</span>
        </label>
        ...
        <div class="version-detail-actions">
          <button class="secondary-button" type="button" data-action="cancel-version-detail">取消</button>
          <button class="primary-button" type="button" data-action="save-version-detail">保存</button>
        </div>
      </div>
    </article>
  `;
...
function renderVersionReadonly(app) {
  const draft = ensureVersionDraft(app);
  const changeGroups = getVersionChangeGroups(app);
  const isPublishing = isAppPublishing(app);
  const isPublished = app.status === "已启用" && !app.pendingPublishChanges && !isPublishing;
  const badgeLabel = isPublishing ? "发版中" : isPublished ? "已发版" : "待申请";
  const badgeClassName = isPublishing
    ? "version-detail-badge version-detail-badge--publishing"
    : isPublished
      ? "version-detail-badge version-detail-badge--published"
      : "version-detail-badge";

  return `
    <article class="info-card page-card version-detail-page-card version-detail-page-card--readonly">
      <div class="version-detail-toolbar">
        ...
        ${
          isPublished || isPublishing
            ? ""
            : '<button class="primary-button" type="button" data-action="confirm-version-publish">确认发版</button>'
        }
      </div>

      <div class="version-detail-page-card__head version-detail-page-card__head--readonly">
        <div class="version-detail-title-row">
          <h1>版本详情</h1>
          <span class="${badgeClassName}">${badgeLabel}</span>
        </div>
      </div>
      ...
      <div class="version-detail-form">
        <section class="version-detail-readonly-field">
          <span class="field-label">应用版本号</span>
          <div class="version-detail-readonly-box">${escapeHtml(draft.version)}</div>
          <span class="field-tip">对用户展示的正式版本号，上一个版本号是 ${escapeHtml(draft.previousVersion)}</span>
        </section>

        <section class="version-detail-readonly-field version-detail-field--notes">
          <span class="field-label">更新说明</span>
          <div class="version-detail-readonly-box version-detail-readonly-box--multiline">${escapeHtml(draft.releaseNotes)}</div>
        </section>

        ${renderVersionChangeContent(changeGroups)}

        ${renderVersionNotifyBlock(app)}
      </div>
    </article>
  `;
}
...
function renderVersionSection(app) {
  if (state.versionComposerOpen) {
    return state.versionDetailReadonly ? renderVersionReadonly(app) : renderVersionComposer(app);
  }

  const versionStatus = getVersionDisplayStatus(app);
  const versionHistory = Array.isArray(app.versionHistory) ? app.versionHistory : [];
      ...
      <div class="version-table">
        ...
        ${
          versionHistory.length
            ? `
              <button class="version-history-toggle" type="button" data-action="toggle-version-history">
                <span class="version-history-toggle__arrow${state.versionHistoryExpanded ? " is-expanded" : ""}">▾</span>
                <span>历史版本 (${versionHistory.length})</span>
              </button>
              ${
                state.versionHistoryExpanded
                  ? `
                    <div class="version-history-list">
                      ${versionHistory
                        .map(
                          (item) => `
                            <div class="version-table-row version-table-row--history">
                              <div>${escapeHtml(item.version)}</div>
                              <div>-</div>
                              ${renderVersionOwner(item.owner)}
...
function renderDetailView() {
  ...
  detailHeaderStatus.textContent = lifecycleMeta.displayText;
  detailHeaderStatus.className = `status-pill ${lifecycleMeta.className}`;
  detailHeaderMeta.textContent = "正式应用@小米科技";
  renderDetailPublishToast(app);
...
function render() {
  ...
  detailView.classList.toggle(
    "detail-view--version-flat",
    state.currentView === "detail" && state.detailSection === "version",
  );
  renderTabPanels();
...
function handleDetailAction(action, capability = "") {
  ...
  if (action === "create-version-from-reminder" || action === "open-version-from-reminder") {
    openVersionComposer(app.id);
    return;
  }

  if (action === "revert-pending-changes") {
    updateAppById(app.id, (current) => {
      restoreCurrentAppToPublishedVersion(current);
      appendActivity(current, "撤回待发布修改", "已恢复到当前已发布版本。");
      return current;
    });
    closeVersionComposer();
    render();
    toast("已恢复到当前已发布版本。", "版本管理");
    return;
  }
  ...
  if (action === "toggle-version-history") {
    state.versionHistoryExpanded = !state.versionHistoryExpanded;
    render();
    return;
  }

  if (action === "back-version-list") {
    closeVersionComposer();
    render();
    return;
  }

  if (action === "toggle-version-notify-picker") {
    state.versionNotifyPickerOpen = !state.versionNotifyPickerOpen;
    render();
    return;
  }

  if (action === "add-version-notify-member") {
    if (!state.versionDraft) {
      return;
    }

    state.versionDraft.notificationMembers = normalizeMemberArray([
      ...(state.versionDraft.notificationMembers || []),
      capability,
    ]);
    render();
    return;
  ...
  if (action === "remove-version-notify-member") {
    if (!state.versionDraft) {
      return;
    }

    state.versionDraft.notificationMembers = normalizeMemberArray(
      (state.versionDraft.notificationMembers || []).filter((item) => item !== capability),
    );
    render();
    return;
  }

  if (action === "cancel-version-detail") {
    closeVersionComposer();
    render();
    return;
  }

  if (action === "save-version-detail") {
    if (!state.versionDraft) {
      return;
    }

    const nextDraft = normalizeVersionDraftConfig(state.versionDraft, app);
    ...
    if (!nextDraft.releaseNotes.trim()) {
      toast("请填写更新说明。", "版本详情");
      return;
    }

    updateAppById(app.id, (current) => {
      current.versionDraft = nextDraft;
      current.versionNoticeMembers = [...nextDraft.notificationMembers];
      current.updatedAt = new Date().toISOString();
      appendActivity(current, "保存版本详情", `已保存版本 ${nextDraft.version} 的详情草稿。`);
      return current;
    });
    state.versionComposerOpen = true;
    state.versionDetailReadonly = true;
    state.versionNotifyPickerOpen = false;
    state.versionDraft = nextDraft;
    render();
    toast("版本详情已保存。", "版本管理");
    return;
  }

  if (action === "confirm-version-publish") {
    const publishDraft = normalizeVersionDraftConfig(state.versionDraft || app.versionDraft, app);
    const previousVersion = app.version;
    const previousPublishedAt = app.updatedAt || new Date().toISOString();
    const publishCompleteAt = new Date(Date.now() + PUBLISH_TRANSITION_DELAY).toISOString();

    state.versionComposerOpen = true;
    state.versionDetailReadonly = true;
    state.versionNotifyPickerOpen = false;
    state.versionDraft = publishDraft;
    const updated = updateAppById(app.id, (current) => {
      current.version = publishDraft.version;
      current.versionDraft = null;
      current.versionNoticeMembers = [...publishDraft.notificationMembers];
      current.versionHistory = normalizeVersionHistory(
        [
      ...
      current.pendingPublishChanges = false;
      current.status = "已启用";
      current.publishStage = "publishing";
      current.publishCompleteAt = publishCompleteAt;
      current.updatedAt = new Date().toISOString();
      current.publishedSnapshot = capturePublishedSnapshot(current);
      appendActivity(current, "确认发布版本", `版本 ${publishDraft.version} 发版中，预计5分钟后生效。`);
      return current;
    });

    if (updated) {
      schedulePublishCompletion(updated.id);
    }

    toast(`版本 ${publishDraft.version} 发版中，预计5分钟后生效。`, "版本管理");
    return;
  }
...
jobEditForm.addEventListener("submit", (event) => {
  ...
  if (trimmed === String(app[field.key] || "").trim()) {
    closeJobEditModal();
    return;
  }

  updateSelectedApp((current) => {
    markPendingPublishChange(current);
...
detailSidebar.addEventListener("click", (event) => {
  ...
  if (button.dataset.section === "version") {
    closeVersionComposer();
  }
...
detailMain.addEventListener("click", (event) => {
  ...
  if (actionButton) {
    handleDetailAction(
      ...
      actionButton.dataset.memberName ||
        actionButton.dataset.collaboratorId ||
        actionButton.dataset.capability ||
        "",
    );
  }
});
...
detailMain.addEventListener("input", (event) => {
  ...
  if (!versionField || !state.versionDraft) {
    return;
  }

  state.versionDraft[versionField.dataset.versionField] = versionField.value;

  const counter = detailMain.querySelector("[data-version-counter]");
  if (counter && versionField.dataset.versionField === "releaseNotes") {
    counter.textContent = `${versionField.value.length}/500`;
  }
});

detailMain.addEventListener("change", (event) => {
  const versionField = event.target.closest("[data-version-field]");
  if (!versionField || !state.versionDraft) {
    return;
  }
...
detailPublishToast.addEventListener("click", (event) => {
  const actionButton = event.target.closest("[data-action]");
  if (!actionButton) {
    return;
  }

  handleDetailAction(actionButton.dataset.action);
});
...
state.apps.forEach((app) => {
  if (isAppPublishing(app)) {
    schedulePublishCompletion(app.id);
  }
});
renderCreateSelectors();
createPreviewIcon();
